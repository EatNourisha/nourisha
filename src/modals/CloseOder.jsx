import africaFood from "../assets/IMG55.png"

const CloseOder = ({data, loading}) => {
    console.log(data)
  return (
    <div>
   {data.map((data) => (
        <div key={data._id}>
          <div key={data.id} className="space-y-4 p-4 md:flex md:space-x-8 md:space-y-0 md:justify-between">
    ...
    {/* <div className="space-y-4 p-4 md:flex md:space-x-8 md:justify-between">
        ...
        <div>
           ...
           <div className="flex justify-between items-center mt-4">
            <span className="bg-green-500 p-2 px-3 rounded-lg text-white text-[10px]">Delivered</span>
            <span className="text-[12px]">12-1-2023</span>
           </div>
        </div>
    </div>
